...
class RubikQLearningAgent:
    ...
    def evaluate(self, num_episodes=100, scramble_lengths=None):
        ...
        ## random agent vs our trained agentttt
        random_results = {length: {"solved": 0, "total": 0} for length in scramble_lengths}
        random_env = RubiksCube2x2()
        ...
        for length in scramble_lengths:
            ...
            for _ in tqdm(range(num_episodes)):
                ...
                while steps < length * 5 + 20:
                    ...
                    self.env.apply_move(self.env.MOVES[action])
                    reward = 10.0 if self.env.is_solved() else -0.1  ## some simplified reward for eval..
                    episode_reward += reward
                    state = self.env.get_compact_state()
                    steps += 1
                ...
                ## now just evaluating with random agent
                random_env.reset()
                random_env.scramble(length)
                random_solved = False
                random_steps = 0
                
                while random_steps < length * 5 + 20:
                    random_action = random.randint(0, len(self.env.MOVES) - 1)
                    random_env.apply_move(self.env.MOVES[random_action])
                    random_steps += 1
                    if random_env.is_solved():
                        random_solved = True
                        break
                
                random_results[length]["total"] += 1
                if random_solved:
                    random_results[length]["solved"] += 1
        ...
        for length in scramble_lengths:
            ...
            random_success_rate = random_results[length]["solved"] / random_results[length]["total"]
            win_rate = success_rate - random_success_rate if results[length]["total"] > 0 else 0.0
            success_rates[length] = success_rate
            win_rates[length] = win_rate
            logger.info(f"Length {length}: Success {success_rate:.2f} ({results[length]['solved']}/{results[length]['total']}), "
                        f"Win vs Random {win_rate:.2f} (Random Success {random_success_rate:.2f})")
        ...
        if episode_rewards:
            ...
            reward_improvement = np.mean(episode_rewards[-num_episodes//10:]) - np.mean(episode_rewards[:num_episodes//10]) if len(episode_rewards) >= num_episodes//10 else 0.0
            logger.info(f"Avg episode reward: {avg_reward:.2f}, Reward improvement: {reward_improvement:.2f}")
        
        return success_rates, win_rates
    ...
    def save(self, filepath, curriculum_state=None):
        ...
        vc_values = np.array([visit_counts_filtered[k] for k in vc_keys], dtype=np.int32)
        vc_key_lengths = []
    ...
    def plot_training_progress(self):
        ...
        axs[0].set_title('Training Rewards vs. Episodes')
        ...
        axs[2].hist(rewards, bins=50, color='lightcoral', alpha=0.7, label='Reward Distribution', log=True)
        ...
        if self.visit_counts:
            state_visits = [np.sum(self.visit_counts[state]) for state in self.visit_counts if np.sum(self.visit_counts[state]) > 0]
            if state_visits:
                axs[3].hist(state_visits, bins=np.logspace(np.log10(max(1, min(state_visits))), np.log10(max(state_visits)), 50), 
                            color='lightblue', alpha=0.7, label='State Visitation Distribution')
                axs[3].set_xscale('log')
                axs[3].set_yscale('log')
                axs[3].set_title('State Visitation Distribution')
                axs[3].set_xlabel('Number of Visits (Log Scale)')
                axs[3].set_ylabel('Frequency (States, Log Scale)')
                axs[3].legend()
                axs[3].grid(True, linestyle='--', alpha=0.5)
            else:
                axs[3].text(0.5, 0.5, 'No state visitation data with non-zero visits', 
                            ha='center', va='center', transform=axs[3].transAxes)
                axs[3].set_title('State Visitation Distribution')
                axs[3].set_xlabel('Number of Visits')
                axs[3].set_ylabel('Frequency (States)')
                axs[3].grid(True, linestyle='--', alpha=0.5)
        else:
            axs[3].text(0.5, 0.5, 'No state visitation data available', 
                        ha='center', va='center', transform=axs[3].transAxes)
            axs[3].set_title('State Visitation Distribution')
            axs[3].set_xlabel('Number of Visits')
            axs[3].set_ylabel('Frequency (States)')
    ...
    def plot_value_function_heatmap(self, max_states=40):
        ...
        ## calculating a score for each state to determine informativeness
        keys = list(self.q_table.keys())
        if not keys:
            logger.info("No states in Q-table for heatmap.")
            return

        ## here just scoring the states by the maximum absolute Q-value
        scores = []
        for key in keys:
            q_values = self.q_table[key]
            score = np.max(np.abs(q_values))
            scores.append((score, key))

        scores.sort(reverse=True)  ## highest first
        selected_states = [key for _, key in scores[:min(max_states, len(scores))]]

        ## create a heatmap of Q-values
        q_values = np.array([self.q_table[k] for k in selected_states])
        moves = self.env.MOVES

        fig, ax = plt.subplots(figsize=(min(18, 1.5 * len(moves)), max(6, 0.4 * len(selected_states))))
        ...
        ax.set_yticklabels([f"State {i+1}" for i in range(len(selected_states))])
        ax.set_title("Q-Value Heatmap (Top Informative States)")
